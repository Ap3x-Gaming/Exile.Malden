...
if (_keyCode in (actionKeys "User4")) exitWith
{
    if !(ExileClientIsHandcuffed || ExileIsPlayingRussianRoulette) then
    {
        ...
        {
            ["SuccessTitleOnly", ["You're already full health"]] call ExileClient_gui_toaster_addTemplateToast;
        }
        else
        {
            if !(alive player) then
            {
                ["ErrorTitleOnly", ["You're dead press esc and respawn."]] call ExileClient_gui_toaster_addTemplateToast;
            }
            else
            {
                if !("Exile_Item_InstaDoc" in (magazines player)) then
                {
                    ["WarningTitleOnly", ["Missing InstaDoc!"]] call ExileClient_gui_toaster_addTemplateToast;
                }
                else
                {
                    if ("Exile_Item_InstaDoc" in (magazines player)) then
