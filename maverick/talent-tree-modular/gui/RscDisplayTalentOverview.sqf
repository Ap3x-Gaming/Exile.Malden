	...
	switch (_mode) do
	{
		case "onLoad":
		{
			...
			if (!life_perksInitialized) exitWith {
				["ErrorTitleAndText", ["Perks!", "Your perks are still being loaded by the server, please try again later"]] call ExileClient_gui_toaster_addTemplateToast;
		...
		case "onButtonPurchaseClick":
		{
			...
			if ((getNumber (_selectedPerkConfig >> "requiredPerkPoints")) > life_currentPerkPoints) exitWith {
				["ErrorTitleAndText", ["Perks!", "You do not have enough perk points to purchase this perk!"]] call ExileClient_gui_toaster_addTemplateToast;
			};
			
			if ((getNumber (_selectedPerkConfig >> "requiredPerkPoints")) > life_currentPerkPoints) then {
				["ErrorTitleAndText", ["Perks!", "You do not have the required level to unlock this perk!"]] call ExileClient_gui_toaster_addTemplateToast;
			};
			...
			["SuccessTitleAndText", ["Perks!", "Perk purchased!"]] call ExileClient_gui_toaster_addTemplateToast;
