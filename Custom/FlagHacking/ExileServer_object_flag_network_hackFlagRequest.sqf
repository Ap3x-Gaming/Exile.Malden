...
{	
	...
	_index = -1;
	{
		if (toLower(_x select 0) isEqualTo toLower(_storedVehicleRandom select 0)) exitWith 
		{
	...
	_vehicleInfo = format["confirmVehicleOwnership:%1", _storedVehicleRandom select 1, _storedVehicleRandom] call ExileServer_system_database_query_selectSingle;
