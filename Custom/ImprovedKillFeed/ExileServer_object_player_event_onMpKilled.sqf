...
switch (_killType) do 
{
	default 
	{
		...
		[
			...
			"%1 died because Gandalf forgot his staff.",
			"%1 died cuz arma fucked him.",
			"%1 Died cause the weed was stronger."
	...
	case 3:
	{
		_countDeath = true;
		_countKill = false;
		_systemChat = format ["%1 died because he crashed!", name _victim];
	...
	case 4:
	{
		_countDeath = false;
		_countKill = false;
		_npcDeath = selectRandom ["was killed in action", "was butchered", "got smoked"]; //Add more messages here to change the killed text
		_systemChat = format ["%1 %2 by an AI!", name _victim, _npcDeath];
