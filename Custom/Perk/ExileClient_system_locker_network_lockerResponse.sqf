...
_lockerAmount = _lockerAmount call ExileClient_util_string_exponentToString;
_lockerLimit = (getNumber(missionConfigFile >> "CfgLocker" >> "maxDeposit"));
if !(isNil "mav_ttm_var_lockerMultiplier") then 
{
	_lockerLimit = _lockerLimit*mav_ttm_var_lockerMultiplier;
};
_lockerLimit = _lockerLimit call ExileClient_util_string_exponentToString;
...
_inventoryAmount = _inventoryAmount call ExileClient_util_string_exponentToString;
