...
{
    case "StealFlag": {
		if !(isNil "mav_ttm_var_stealMultiplier") then 
		{
			_duration = _duration / mav_ttm_var_stealMultiplier;
		};
	};
    case "GrindLock":
	{
		if !(isNil "mav_ttm_var_grindMultiplier") then 
		{
			_duration = _duration / mav_ttm_var_grindMultiplier;
		};
	};
    case "HackLock": 
	{
		if !(isNil "mav_ttm_var_hackMultiplier") then 
		{
			_duration = _duration / mav_ttm_var_hackMultiplier;
		};
	};
    case "HackFlag": 
	{
	...
	case "HotwireVehicle": 
	{
		if !(isNil "mav_ttm_var_pickMultiplier") then 
		{
			_duration = _duration / mav_ttm_var_pickMultiplier;
		};
	};
	case "RepairVehicle": 
	{
		if !(isNil "mav_ttm_var_repairMultiplier") then 
		{
			_duration = _duration / mav_ttm_var_repairMultiplier;
		};
	};
	case "PlantChargeConcrete": 
	{
		if !(isNil "mav_ttm_var_breachMultiplier") then 
		{
			_duration = _duration / mav_ttm_var_breachMultiplier;
		};
	};
	case "PlantChargeMetal": 
	{
		if !(isNil "mav_ttm_var_breachMultiplier") then 
		{
			_duration = _duration / mav_ttm_var_breachMultiplier;
